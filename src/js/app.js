...
function scrollNav() {
    ...
    enlaces.forEach((enlace) => {
        enlace.addEventListener('click', function (evt) {
            evt.preventDefault();
            const seccionScroll = evt.target.attributes.href.value;
...
function crearGaleria() {
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        imagen.innerHTML = `
             <source srcset="build/img/thumb/${i}.jpg" type="image/avif">
...
function mostrarImagen(id) {
    const imagen = document.createElement('PICTURE');
    imagen.innerHTML = `
             <source srcset="build/img/grande/${id}.jpg" type="image/avif">
